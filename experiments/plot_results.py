...
Graphs produced (saved to experiments/results/plots/):
  ...
  2. replica_count.png      — replica count over time (from Prometheus)
  3. throughput.png         — successful requests/s over time
  4. dropped.png            — dropped + timeout requests over time
  5. summary_bar.png        — bar chart: overall p99, drop rate, avg replicas
...
# ── Paths ─────────────────────────────────────────────────────────────
RESULTS_DIR = Path(__file__).parent / "load-tester" / "results"
PLOTS_DIR   = Path(__file__).parent / "experiments" / "results" / "plots"
...
SLO_MS = 400   # 400ms latency SLO line
...
def load_summaries() -> dict[str, pd.DataFrame]:
    ...
    for name in EXPERIMENTS:
        ...
        if not path.exists():
            ...
            continue
        df = pd.read_csv(path)
        df["second_min"] = df["second"] / 60   # convert to minutes for x-axis
        dfs[name] = df
...
def plot_p99_latency(dfs: dict):
    ...
    for name, df in dfs.items():
        ...
        # Smooth with a 5s rolling average to reduce noise
...
# ─────────────────────────────────────────────────────────────────────
# Plot 2 — Throughput (successful req/s)
# ─────────────────────────────────────────────────────────────────────

def plot_throughput(dfs: dict):
    fig, ax = plt.subplots(figsize=(12, 5))

    # Scheduled QPS from any experiment (same trace for all)
...
# ─────────────────────────────────────────────────────────────────────
# Plot 3 — Dropped + timeout requests over time
# ─────────────────────────────────────────────────────────────────────
...
def plot_summary_bars(dfs: dict):
    ...
    for name in names:
        ...
        # Overall p99 = 99th percentile of per-second p99 values
...
def main():
    ...
    print("Done. Open experiments/results/plots/ to view the graphs.")
...
# ─────────────────────────────────────────────────────────────────────
# Plot 5 — Replica count over time
# ─────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────
# Plot 6 — CPU cores over time  (professor's required metric)
# ─────────────────────────────────────────────────────────────────────
